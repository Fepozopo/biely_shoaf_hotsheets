...
def smd_stock():
    print('Matching Stock...')

    wb1 = openpyxl.load_workbook('./xlsx/SMD-IM_StockStatus.xlsx')
    ws1 = wb1['Sheet1']
        ...
        for row_ws1 in range(ws1_pointer, ws1.max_row + 1): # ------- Iterate through rows in ws1 starting from ws1_pointer -------
            ...
            if sku_ws3.strip() in sku_ws1.strip():
                if ws1[row_ws1 + 2][on_hand_col].value is None:
                    # Update 'F' (on_hand), 'I' (on_po), and 'K' (on_so, on_bo) in ws3
                    ws3[f'F{row_ws3}'].value = ws1[row_ws1 + 1][on_hand_col].value
                    ws3[f'I{row_ws3}'].value = ws1[row_ws1 + 1][on_po_col].value
                    ws3[f'K{row_ws3}'].value = ws1[row_ws1 + 1][on_so_col].value + ws1[row_ws1 + 1][on_bo_col].value
                    ...
                    ws3[f'K{row_ws3}'].value = ws1[row_ws1 + 2][on_so_col].value + ws1[row_ws1 + 2][on_bo_col].value
...
def smd_stock_holiday():
    print('Matching Stock Holiday...')
    ...
    for row_ws3 in range(start_row_ws3, ws3.max_row + 1):
        ...
        for row_ws1 in range(ws1_pointer, ws1.max_row + 1): # ------- Iterate through rows in ws1 starting from ws1_pointer -------
            ...
            if sku_ws3.strip() in sku_ws1.strip():
                if ws1[row_ws1 + 2][on_hand_col].value is None:
                    # Update 'D' (on_hand), 'F' (on_po), and 'H' (on_so, on_bo) in ws3
                    ws3[f'D{row_ws3}'].value = ws1[row_ws1 + 1][on_hand_col].value
                    ...
                    # Update 'D' (on_hand), 'F' (on_po), and 'H' (on_so, on_bo) in ws3
                    ws3[f'D{row_ws3}'].value = ws1[row_ws1 + 2][on_hand_col].value
